...
def check_rate_limit(request: Request, rate: str, namespace: str, key: str | None = None) -> None:
    try:
        request.app.state.rate_limiter.check(rate, namespace, _client_ip(request) if key is None else key)
