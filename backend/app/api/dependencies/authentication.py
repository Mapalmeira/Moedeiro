...
from app.application.registry.use_cases.authentication import authenticate_session
...
def require_authenticated_user(request: Request) -> User:
    ...
    try:
        return authenticate_session(request.app.state.databases.open_registry, token, int(time.time()))
