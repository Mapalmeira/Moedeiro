...
def create_ledger_category(ledger_uuid: UUID, payload: CreateCategoryRequest, request: Request, ledger: GrantedLedger) -> CategoryResponse:
    try:
...
def get_ledger_category_tree(ledger_uuid: UUID, request: Request, ledger: GrantedLedger) -> list[CategoryTreeNodeResponse]:
    try:
        tree = get_category_tree(ledger_unit_of_work_factory(request, ledger))
    except CategoryTreeSizeExceededError as error:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Category limit exceeded") from error
...
def get_ledger_category(ledger_uuid: UUID, category_uuid: UUID, request: Request, ledger: GrantedLedger) -> CategoryResponse:
    try:
        category = get_category(ledger_unit_of_work_factory(request, ledger), category_uuid)
    except CategoryNotFoundError as error:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Category not found") from error
    return CategoryResponse.from_category(category)
...
def update_ledger_category(ledger_uuid: UUID, category_uuid: UUID, payload: UpdateCategoryRequest, request: Request, ledger: GrantedLedger) -> CategoryResponse:
    try:
...
def delete_ledger_category(ledger_uuid: UUID, category_uuid: UUID, request: Request, ledger: GrantedLedger) -> None:
    try:
        delete_category(ledger_unit_of_work_factory(request, ledger), category_uuid)
    except CategoryNotFoundError as error:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Category not found") from error
