...
class SqliteRecoveryCodeRepository(RecoveryCodeRepository):
    _columns = "uuid, user_uuid, code_hash, created_at, used_at"
    ...
    def create(self, user_uuid: UUID, code_hash: bytes, created_at: int) -> RecoveryCode:
        ...
        self.connection.execute(
            "INSERT INTO recovery_code(uuid, user_uuid, code_hash, created_at, used_at) VALUES (?, ?, ?, ?, ?)",
            (recovery_code.uuid.bytes, recovery_code.user_uuid.bytes, recovery_code.code_hash, recovery_code.created_at, recovery_code.used_at),
        )
    ...
    def get_active_by_user(self, user_uuid: UUID) -> RecoveryCode | None:
        row = self.connection.execute(f"SELECT {self._columns} FROM recovery_code WHERE user_uuid = ? AND used_at IS NULL", (user_uuid.bytes,)).fetchone()
        return None if row is None else self._to_model(row)

    def consume(self, uuid: UUID, used_at: int) -> bool:
        cursor = self.connection.execute("UPDATE recovery_code SET used_at = ? WHERE uuid = ? AND used_at IS NULL AND created_at <= ?", (used_at, uuid.bytes, used_at))
    ...
    def delete_inactive_before(self, timestamp: int) -> int:
        cursor = self.connection.execute("DELETE FROM recovery_code WHERE used_at <= ?", (timestamp,))
        return cursor.rowcount
