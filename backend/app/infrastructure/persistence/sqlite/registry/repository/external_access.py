...
class SqliteExternalAccessRepository(ExternalAccessRepository):
    _columns = "uuid, name, token_hash"

    def __init__(self, connection: sqlite3.Connection):
        self.connection = connection

    def create(self, name: str, token_hash: bytes) -> ExternalAccess:
        access = ExternalAccess(uuid=uuid4(), name=name, token_hash=token_hash)
        self.connection.execute("INSERT INTO ledger_grantee(uuid) VALUES (?)", (access.uuid.bytes,))
        try:
            self.connection.execute(
                "INSERT INTO external_access(uuid, name, token_hash) VALUES (?, ?, ?)",
                (access.uuid.bytes, access.name, access.token_hash),
            )
    ...
    def list_all(self) -> list[ExternalAccess]:
        rows = self.connection.execute(f"SELECT {self._columns} FROM external_access ORDER BY name ASC, uuid ASC").fetchall()
