...
class SqliteRegistrySchemaConstraintsTest(unittest.TestCase):
    ...
    def test_enforces_recovery_code_usage_timestamp_expiration_and_one_active_code(self) -> None:
        ...
        self.connection.execute("INSERT INTO recovery_code VALUES (?, ?, ?, 30, 50, NULL)", (code_uuid, self.user_uuid, b"c" * 32))
        with self.assertRaises(sqlite3.IntegrityError):
            self.connection.execute("UPDATE recovery_code SET used_at = 29 WHERE uuid = ?", (code_uuid,))
        with self.assertRaises(sqlite3.IntegrityError):
            self.connection.execute("INSERT INTO recovery_code VALUES (?, ?, ?, 31, 51, NULL)", (uuid4().bytes, self.user_uuid, b"d" * 32))

        with self.assertRaises(sqlite3.IntegrityError):
            self.connection.execute("UPDATE recovery_code SET used_at = 50 WHERE uuid = ?", (code_uuid,))
