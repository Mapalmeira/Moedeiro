...
class SqliteRegistryUnitOfWorkTest(unittest.TestCase):
    ...
    def test_commit_persists_changes_after_the_scope_ends(self) -> None:
        ...
        with SqliteRegistryUnitOfWork(self.database) as unit_of_work:
            unit_of_work.ledger_repository.create(uuid4(), "Committed", "committed.sqlite", "lucide:BookOpen", b"\x80\x80\x80", 10)
            unit_of_work.commit()
    ...
    def test_explicit_rollback_discards_pending_changes(self) -> None:
        ...
        with SqliteRegistryUnitOfWork(self.database) as unit_of_work:
            unit_of_work.ledger_repository.create(uuid4(), "Rolled back", "rolled-back.sqlite", "lucide:BookOpen", b"\x80\x80\x80", 10)
    ...
    def test_exit_without_commit_rolls_back_pending_changes(self) -> None:
        ...
        with SqliteRegistryUnitOfWork(self.database) as unit_of_work:
            unit_of_work.ledger_repository.create(uuid4(), "Uncommitted", "uncommitted.sqlite", "lucide:BookOpen", b"\x80\x80\x80", 10)
    ...
    def test_exception_rolls_back_pending_changes(self) -> None:
        ...
        with self.assertRaises(RuntimeError):
            with SqliteRegistryUnitOfWork(self.database) as unit_of_work:
                unit_of_work.ledger_repository.create(uuid4(), "Failing", "failing.sqlite", "lucide:BookOpen", b"\x80\x80\x80", 10)
                raise RuntimeError("expected failure")
