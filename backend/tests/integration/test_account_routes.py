...
class AccountRoutesTest(unittest.TestCase):
    ...
    def test_create_and_get_return_the_persisted_account(self) -> None:
        ...
        response = get_ledger_account(self.ledger.uuid, created.uuid, self.request, self.ledger)
    ...
    def test_list_returns_the_entire_collection_without_page_limits(self) -> None:
        initial = list_ledger_accounts(self.ledger.uuid, self.request, self.ledger)
        created = [self.create_account(name) for name in ("Charlie", "Alpha", "Bravo", "Delta")]
        self.assertEqual(list_ledger_accounts(self.ledger.uuid, self.request, self.ledger), initial + created)
    ...
    def test_creation_limit_can_be_reused_after_deletion(self) -> None:
        ...
        delete_ledger_account(self.ledger.uuid, created[-1].uuid, self.request, self.ledger)
        self.create_account("Replacement")
        self.assertEqual(len(list_ledger_accounts(self.ledger.uuid, self.request, self.ledger)), 300)
    ...
    def test_update_rejects_an_unknown_account_or_unavailable_name(self) -> None:
        ...
        with self.assertRaises(HTTPException) as missing:
            update_ledger_account(self.ledger.uuid, uuid4(), payload, self.request, self.ledger)
        with self.assertRaises(HTTPException) as conflict:
            update_ledger_account(self.ledger.uuid, other.uuid, payload, self.request, self.ledger)

        self.assertEqual(missing.exception.status_code, 404)
        self.assertEqual(conflict.exception.status_code, 409)
        self.assertEqual(get_ledger_account(self.ledger.uuid, existing.uuid, self.request, self.ledger), existing)

    def test_delete_removes_an_unused_account(self) -> None:
        account = self.create_account()

        self.assertIsNone(delete_ledger_account(self.ledger.uuid, account.uuid, self.request, self.ledger))

        with self.assertRaises(HTTPException) as raised:
            get_ledger_account(self.ledger.uuid, account.uuid, self.request, self.ledger)
    ...
    def test_delete_returns_conflict_when_a_movement_uses_the_account(self) -> None:
        ...
        with self.assertRaises(HTTPException) as raised:
            delete_ledger_account(self.ledger.uuid, account.uuid, self.request, self.ledger)
