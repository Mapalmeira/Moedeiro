...
class AccountUseCasesTest(unittest.TestCase):
    ...
    def test_create_rejects_an_unknown_currency(self) -> None:
        with self.assertRaises(CurrencyNotFoundError):
            create_account(self.open_ledger, "Checking", None, uuid4(), "WalletCards", b"\x40\x50\x60")
    ...
    def test_create_rejects_invalid_data_without_persisting_it(self) -> None:
        with self.assertRaises(ValidationError):
            create_account(self.open_ledger, "", None, self.currency.uuid, "WalletCards", b"\x40\x50\x60")
    ...
    def test_update_changes_mutable_fields_but_preserves_currency(self) -> None:
        ...
        updated = update_account(self.open_ledger, account.uuid, "Savings", None, "PiggyBank", b"\xaa\xbb\xcc")
    ...
    def test_update_rejects_an_unknown_account_or_unavailable_name(self) -> None:
        ...
        with self.assertRaises(AccountNotFoundError):
            update_account(self.open_ledger, uuid4(), "Missing", None, "WalletCards", b"\x40\x50\x60")
        with self.assertRaises(AccountNameUnavailableError):
            update_account(self.open_ledger, other.uuid, existing.name, None, "WalletCards", b"\x40\x50\x60")
    ...
    def test_delete_rejects_an_account_selected_by_a_budget(self) -> None:
        ...
        with self.open_ledger() as unit_of_work:
            category = unit_of_work.category_repository.create("Food", "Utensils", b"\x80\x80\x80", None)
            budget = unit_of_work.budget_repository.create(category.uuid, self.currency.uuid, 20, 30, "Monthly", "Food budget", 100, "ReceiptText", b"\x80\x80\x80")
