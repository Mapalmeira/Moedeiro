...
class SqliteRememberSessionRepositoryTest(RegistryRepositoryTestCase):
    def test_create_can_be_read_by_uuid_and_token_hash(self) -> None:
        ...
        session = self.remember_session_repository.create(user.uuid, b"t" * 32, 30)
    ...
    def test_create_requires_an_existing_user_and_unique_token_hash(self) -> None:
        user = self.create_user()
        self.remember_session_repository.create(user.uuid, b"t" * 32, 30)

        with self.assertRaises(sqlite3.IntegrityError):
            self.remember_session_repository.create(user.uuid, b"t" * 32, 31)
        with self.assertRaises(sqlite3.IntegrityError):
            self.remember_session_repository.create(uuid4(), b"u" * 32, 30)
    ...
    def test_rotate_rejects_expired_and_revoked_sessions(self) -> None:
        ...
        revoked = self.remember_session_repository.create(user.uuid, b"r" * 32, 30, 100)
    ...
    def test_revoke_by_user_preserves_existing_revocation_and_excludes_other_user(self) -> None:
        user = self.create_user()
        first = self.remember_session_repository.create(user.uuid, b"a" * 32, 30)
        second = self.remember_session_repository.create(user.uuid, b"b" * 32, 30)
        other = self.remember_session_repository.create(self.create_user().uuid, b"c" * 32, 30)
