...
class SqliteUserPreferencesRepositoryTest(RegistryRepositoryTestCase):
    def test_save_creates_preferences(self) -> None:
        ...
        preferences = self.preferences_repository.save(user.uuid, "pt-BR", "LIGHT")
    ...
    def test_save_replaces_all_preferences_for_existing_user(self) -> None:
        user = self.create_user()
        self.preferences_repository.save(user.uuid, "pt-BR", "LIGHT")

        updated = self.preferences_repository.save(user.uuid, "en", "DARK")
    ...
    def test_save_requires_an_existing_user(self) -> None:
        with self.assertRaises(sqlite3.IntegrityError):
            self.preferences_repository.save(uuid4(), "pt-BR", "LIGHT")
