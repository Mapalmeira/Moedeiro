...
class ApplicationFactoryTest(unittest.TestCase):
    def test_create_app_initializes_and_exposes_its_configuration_and_databases(self) -> None:
        with TemporaryDirectory() as temporary_directory:
            ...
            self.assertTrue({"/api/authentication/login", "/api/authentication/logout", "/api/ledgers", "/api/ledgers/{ledger_uuid}", "/api/ledgers/{ledger_uuid}/accounts", "/api/ledgers/{ledger_uuid}/accounts/{account_uuid}", "/api/ledgers/{ledger_uuid}/budgets", "/api/ledgers/{ledger_uuid}/budgets/{budget_uuid}", "/api/ledgers/{ledger_uuid}/budgets/{budget_uuid}/status", "/api/ledgers/{ledger_uuid}/categories", "/api/ledgers/{ledger_uuid}/categories/tree", "/api/ledgers/{ledger_uuid}/categories/{category_uuid}", "/api/ledgers/{ledger_uuid}/currencies", "/api/ledgers/{ledger_uuid}/currencies/{currency_uuid}", "/api/ledgers/{ledger_uuid}/events", "/api/ledgers/{ledger_uuid}/events/{event_uuid}", "/api/password/change", "/api/password/recovery", "/api/registration"}.issubset(application.openapi()["paths"]))
