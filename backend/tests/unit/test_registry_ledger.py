    ...
    def test_accepts_nonempty_path_without_maximum_length(self) -> None:
        for path in ("x", "x" * 10000):
            with self.subTest(path_length=len(path)):
                ledger = Ledger(uuid=uuid4(), path=path, icon="BookOpen", color_code=b"\x00\x00\x00")
                self.assertEqual(ledger.path, path)

    def test_rejects_empty_path(self) -> None:
        with self.assertRaises(ValidationError):
            Ledger(uuid=uuid4(), path="", icon="BookOpen", color_code=b"\x00\x00\x00")
    ...
    def test_rejects_icon_and_color_outside_limits(self) -> None:
        ...
        for field, value in invalid_values:
            with self.subTest(field=field, length=len(value)):
                values = {"uuid": uuid4(), "path": "ledger.sqlite", "icon": "BookOpen", "color_code": b"\x00\x00\x00"}
                values[field] = value
                with self.assertRaises(ValidationError):
                    Ledger(**values)
