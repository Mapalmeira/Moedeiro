...
@Component({
  ...
  template: `
    <header class="auth-card__header">
      <span class="auth-card__icon ui-projected-icon"><app-icon [name]="icon()" [size]="25" /></span>
      <h2>{{ title() }}</h2>
    </header>
  ...
  `,
  styles: `
    :host { display: flex; flex-direction: column; min-width: 0; padding: clamp(22px, 3vw, 30px); border: 2px solid var(--line-strong); border-radius: var(--radius-card); background: var(--surface); filter: var(--shadow-hard); }
    ...
    .auth-card__icon { display: inline-grid; place-items: center; width: 46px; height: 46px; flex: 0 0 46px; border: 2px solid var(--line-strong); border-radius: 5px; background: var(--card-accent); color: #060606; }
