...
export class LedgerAppearanceFieldsComponent {
  ...
  constructor() {
    effect((onCleanup) => {
      const icon = this.iconControl(), color = this.colorControl();
      const syncIcon = (value: string) => { this.currentIcon.set(value); this.iconMode.set(isEncodedLucideLedgerIcon(value) ? 'lucide' : 'unicode'); };
      const syncColor = (value: string) => { if (COLOR_PATTERN.test(value)) this.color.set(value); };
      untracked(() => { syncIcon(icon.value); syncColor(color.value); });
      const iconSubscription = icon.valueChanges.subscribe(syncIcon);
      const colorSubscription = color.valueChanges.subscribe(syncColor);
      onCleanup(() => { iconSubscription.unsubscribe(); colorSubscription.unsubscribe(); });
    });
  }
  ...
  setColor(event: Event): void { const value = (event.target as HTMLInputElement).value.toUpperCase(); this.colorControl().setValue(value); this.colorControl().markAsDirty(); this.color.set(value); }
  setColorText(event: Event): void { const value = (event.target as HTMLInputElement).value.toUpperCase(); this.colorControl().setValue(value); this.colorControl().markAsDirty(); if (COLOR_PATTERN.test(value)) this.color.set(value); }
}
