...
export function formatEventDate(timestampSeconds: number, timezone: string): string {
  let formatter = eventDateFormatters.get(timezone);
  if (!formatter) {
    formatter = new Intl.DateTimeFormat(undefined, {
      timeZone: timezone,
      year: 'numeric', month: '2-digit', day: '2-digit',
    });
    eventDateFormatters.set(timezone, formatter);
  }
  return formatter.format(new Date(timestampSeconds * 1000));
}

export function formatEventTime(timestampSeconds: number, timezone: string): string {
